...
open class ImplicitGrantFlow: AuthorizationGrantFlow, @unchecked Sendable {
    ...
    open func validate(_ authorizationResponse: AuthorizationResponse) throws {
        ...
        guard authorizationResponse.state == self.state else {
            ...
            throw OneSoftwareIdentityKitError.authenticationFailed(reason: OneSoftwareIdentityKitError.Reason.invalidAuthorizationResponse)
        }
    }
    ...
    public func authenticate(handler: @escaping @Sendable (AccessTokenResponse?, Error?) -> Void) {
        ...
        self.perform(authorizationURLRequest, redirectURI: self.redirectURI) { [weak self] (redirectRequest) throws -> Bool in
            ...
            guard let _self = self else {
                ...
                throw fail(with: OneSoftwareIdentityKitError.authenticationFailed(reason: OneSoftwareIdentityKitError.Reason.general(message: "Flow was deallocated")))
            }
